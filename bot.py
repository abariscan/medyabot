...
def video_indir(url, dosya_adi, sadece_ses=False):
    ...
    ydl_opts = {
        ...
        'extractor_args': {'youtube': {'player_client': ['android', 'web']}, 'twitter': {'api': 'v2'}},
    }
    if sadece_ses:
...
async def buton_tiklama(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        with open(final_dosya, 'rb') as f:
            if sadece_ses: await query.message.reply_audio(audio=f, caption="🎵 Ses hazır!")
            else: await query.message.reply_video(video=f, supports_streaming=True, caption="📹 Videonuz hazır!")
...
async def isleyici(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Desteklenen sitelere twitter ve x.com eklendi
    ...
    if any(site in url for site in desteklenenler):
        keyboard = [[InlineKeyboardButton("📹 Video İndir", callback_data=f"vid|{url}"),
                     InlineKeyboardButton("🎵 MP3 İndir", callback_data=f"aud|{url}")]]
        await update.message.reply_text("📥 Ne indirmek istersin?", reply_markup=InlineKeyboardMarkup(keyboard))
